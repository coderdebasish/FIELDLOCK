...
// Generates: 5 facilities, SlotResource rows for 7 days,
//            20 students with varied AllocationScores + booking history

import { PrismaClient, Role, SlotStatus, BookingStatus } from "@prisma/client";
import bcrypt from "bcryptjs";
import { generateSlotTimes } from "../lib/utils";
...
const FACILITIES = [
  {
    ...
    imageUrl: "/images/badminton-a.jpg",
    openTime: "06:00",
    closeTime: "22:00",
    slotDuration: 60,
  },
  {
    ...
    imageUrl: "/images/badminton-b.jpg",
    openTime: "08:00",
    closeTime: "21:00",
    slotDuration: 60,
  },
...
function getDaysFromToday(offset: number): Date {
  const d = new Date();
...
async function main() {
  console.log("🌱 Seeding FIELDLOCK database...");
  ...
  // Create SlotResource rows: 7 days past + 14 days future
  let slotCount = 0;
  const createdSlots: Array<{ id: string; facilityId: string; slotDate: Date; startTime: string }> = [];
  ...
  // Seed historical bookings (past 7 days)
  const pastSlots = createdSlots.filter((s) => s.slotDate < new Date());
  let bookingCount = 0;

  for (const slot of pastSlots) {
    // ~60% occupancy for past slots
    if (Math.random() > 0.4) {
  ...
  // Seed a few future bookings for demo (Peak: 6 PM slots on badminton court A)
  const badmintonA = facilities.find((f) => f.name === "Badminton Court A")!;
  const tomorrowSlots = createdSlots.filter(
    (s) =>
      s.facilityId === badmintonA.id &&
      s.slotDate.toDateString() === getDaysFromToday(1).toDateString() &&
      s.startTime === "18:00"
  );
  ...
  if (tomorrowSlots.length > 0) {
    ...
    await prisma.booking.create({
      data: {
        ...
        idempotencyKey: `demo-booking-peak-slot`,
        noShowProb: 0.12,
        qrToken: `FL-DEMO-${Date.now()}`,
      },
    });
    ...
    for (let i = 1; i <= 3; i++) {
      await prisma.waitlist.create({
        data: {
          ...
          rankScore: students[i].allocationScore,
          status: "WAITING",
        },
      });
    }
    console.log("✅ Demo peak slot booked + 3 on waitlist");
  }

  // Seed demand predictions
  for (const facility of facilities) {
    for (let dayOffset = 0; dayOffset <= 7; dayOffset++) {
      ...
      for (let hour = 6; hour < 22; hour++) {
        ...
        const demand = isPeak
          ? facility.capacity * (1.5 + Math.random() * 1.5)
          : facility.capacity * (0.2 + Math.random() * 0.6);
        const ratio = demand / facility.capacity;

        let recommendation: string | null = null;
        if (ratio > 2.0) {
          recommendation = `Demand predicted at ${ratio.toFixed(1)}× capacity for ${facility.name} at ${hour}:00. Suggested actions: (1) Open alternate facility if available. (2) Send early-bird notification to redirect 30% demand to ${hour - 1}:00. (3) Extend waitlist capacity.`;
        } else if (ratio < 0.3 && isPeak) {
          recommendation = `${facility.name} underutilised at ${hour}:00 (${Math.round(ratio * 100)}% capacity). Consider promoting this slot to registered students.`;
        }
  ...
  console.log("✅ Demand predictions seeded for 7 days");

  console.log("\n🎉 Seed complete!");
  console.log("─────────────────────────────────────────");
  console.log("Admin:    admin@iitg.ac.in / admin123");
  console.log("Students: 220101001@iitg.ac.in through 220101020@iitg.ac.in / pass123");
  console.log("─────────────────────────────────────────");
}
